...
| ID        | 規格                                                 | 驗收條件                                                                                                                                        |
| --------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-F-001 | 初始畫面依預設機關與日期範圍載入最新成功發布的標案。 | 預設機關＝國防部、日期範圍＝一週、搜尋固定為空且忽略任意 URL query；有效資料載入後 KPI 與列表顯示符合預設篩選的筆數，並依公告日期由新到舊排序。 |
| REQ-F-002 | 支援案名、案號及招標方式篩選。                       | 清除篩選後恢復全資料；搜尋忽略大小寫與前後空白。                                                                                                |
| REQ-F-003 | 顯示來源、查詢模式、擷取時間、筆數與新鮮度。         | 超過兩小時顯示「資料可能過期」（若擷取時間為六日，顯示「週末例行暫停同步（下一版：週一 00:00）」）；擷取時間為最近一次成功同步時間。            |
...
| REQ-F-008 | 提供「當日／一週／一個月」日期範圍篩選。             | 對已載入資料本地端篩選 `announcedDate`，不觸發任何額外網路請求；預設「一週」；篩選結果依公告日期由新到舊排序。                                  |
| REQ-F-009 | 提供資料來源說明，列出收錄機關白名單與同步頻率。     | 頁面上有可展開面板，列出全部白名單機關與排程時間；不觸發任何額外網路請求。                                                                      |
