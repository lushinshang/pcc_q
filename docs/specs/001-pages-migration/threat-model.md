...
| HTML 結構漂移／偽造零筆訊號                | fixture regression、拒絕比例 fail closed；合法零筆須同時具備 `td.tb_b2` 內容節點零筆文字、無標案資料列與 `#pagebanner=0`，script／註解／單一訊號皆拒絕 | PAR-T-003／003b～003e／006、PIPE-T-001～002 |
...
| 不設機關限制後的請求量／資料量暴增         | `MAX_PCC_PAGES`、`MAX_TOTAL_SCANNED_ROWS` 雙層安全上限；最後允許頁仍有下一頁時拋錯，不靜默發布截斷資料；白名單過濾收斂發布範圍                         | PAG-T-005／007／011、FETCH-T-008、PAR-T-009 |
| Bootstrap orchestration 未受測試監督       | `runFetchPipeline()` 注入依賴；null／零筆基準觸發回填；回填失敗 fallback；雙重失敗時在寫檔前中止；CLI 與 pipeline 分離                                 | FETCH-T-001～008                            |
